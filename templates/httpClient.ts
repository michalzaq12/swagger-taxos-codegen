...
export interface IHttpClientOptions {
    ...
    wrapRequest?<T>(request: () => Promise<T>, config: IApiRequestConfig): Promise<T>;
}
...
function createHttpClient(options: IHttpClientOptions) {
    return {
        async request(config: IApiRequestConfig) {
            ...
            const userSignal = config.signal // Store userSignal outside wrapper (without this, retry will not work after a TimeoutError)
